...
GBA Go Operations Copilot helps reservation mobility operators understand abnormal orders, inspect fare and dispatch evidence, receive AI-assisted recommendations, and approve or reject high-risk actions with a clear audit trail.
...
The main challenge was scope control. GBA Go is a larger mobility system, but a hackathon submission needs a crisp story and a safe demo. We had to separate production evidence from submission-safe artifacts, avoid exposing secrets, and make the Copilot useful without letting AI silently execute risky operational changes.

## Accomplishments

- Built a clear operator workflow: explain, recommend, approve or reject, audit.
- Kept human-in-the-loop governance for high-risk mobility actions.
- Added synthetic demo data covering normal dispatch, no-driver exception, driver restriction review, and route verification.
...
- Included sanitized evidence from the pre-existing GBA Go backend production certification: a 20 QPS x 3600 second run with 72,000 completed requests, 72,000 successful requests, zero timeouts, zero state inconsistencies, and TSS 3.849986.
- The repository includes `TEST_REPORT.md`, `SECURITY_SCAN_REPORT.md`, and `CODEX_CONTRIBUTIONS.md` as supporting evidence.
...
1. Show `trip_demo_001` as a normal assigned reservation.
2. Show `trip_demo_002` as a no-driver dispatch exception.
3. Show `trip_demo_003` as a driver restriction review.
4. Click Recommend Action, then Approve Last Action.
5. Show the audit log separating AI recommendation from operator decision.
6. Click Explain System to show sanitized 20 QPS x 3600s stability evidence.
...
TODO: Add public YouTube URL after recording.

## Repository URL

TODO: Add GitHub repository URL after pushing.

## Codex Session / Feedback

Use this current Codex task/session if Devpost asks for the Codex evidence. If the form specifically requires `/feedback`, run that in the Codex task UI and paste the returned id.
